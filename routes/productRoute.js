...
router.post('/add-product', async(req, res)=> {
    try{
        ...
        const existsProduct = await ProductModel.findOne({productId: productId, verified: true});
        if(existsProduct) {
            console.log("This product exist in our records.");
            res.status(400).json({message: "Product already exists", existsProduct});
        } else {
...
//             const existsProduct = await ProductModel.findOne({ productId: product.productId, verified: true });
            
//             if (existsProduct) { //if products already exists, log message will display.
//                 console.log(`Product ${product.productId} already exists.`);
//             // } else {
//                 // const newProduct = new ProductModel({ ...product, verified: true });
//                 // await newProduct.save(); //save new product/products
//                 // console.log(`Product ${product.productId} added successfully.`);
//             }
//         }
         
//         const newProduct = new ProductModel({ productId: req.body.productId, verified: true });
//         await newProduct.save(); //save new product/products
...
router.post('/add-multiple-products', async(req, res)=> {
    try {
        ...
        return res.status(200).json({ message: "All products added successfully."});
...
router.post('/delete-product', async(req, res)=> {
    try{
        ...
        const deletedProduct = await ProductModel.findOneAndDelete({productId: req.body.productId});
        if(deletedProduct) {
